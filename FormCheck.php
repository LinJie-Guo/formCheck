...
class FormCheck
{
    ...
    public function password($password, $strength = 1)
    {
        if ($strength == 1) {
            ...
            if (!preg_match($pattern, $password)) {
                return $this->result("长度不小于8个字符,至少含有一个数字,至少含有一个小写字母,至少含有一个大写字母,可以有特殊字符，也可以没有");
            } else {
                return $this->result("验证通过！", 1);
            }
        }
     ...
     * @param $mobile
     * @param $code
     * @param $codeType 验证码类型：1注册，2密码找回，3密码修改,4预约看墓
     * @return mixed
     */
    ...
    //数据返回
    public function result($msg, $code = 0, $data = array())
    {
        $data['msg'] = $msg;
        $data['code'] = $code;
        $data['data'] = $data;
